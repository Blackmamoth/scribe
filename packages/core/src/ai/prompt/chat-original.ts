...
- Brand information → \`brand = { name: "Company Name", primaryColor: "#0066FF", ... }\`
...
- Brand defaults (handled via default values in brand object)
...
export default function Email({
  ...
  // Brand object with actual values and defaults
  const brand = {
    name: brandData?.name || "Your Company",
    logoUrl: brandData?.logoUrl || "",
    tagline: brandData?.tagline || "",
    websiteUrl: brandData?.websiteUrl || "https://example.com",
    primaryColor: brandData?.primaryColor || "#0066FF",
    secondaryColor: brandData?.secondaryColor || "#64748B"
  };
...
export function buildScribeUserPrompt({
	...
	// Create brand object with defaults for the prompt
	const brandData = {
...
BRAND DATA FOR EMAIL TEMPLATES:
When creating email templates, use this brand data with defaults:
\`\`\`typescript
const brand = {
  name: "${brandData.name}",
  logoUrl: "${brandData.logoUrl}",
...
- Define EmailProps interface with all dynamic fields (including optional brand prop)
- Provide default values for EVERY prop
- Define brand object with actual values and defaults (no template variables)
- Check if brand fields exist before using them in the email template
